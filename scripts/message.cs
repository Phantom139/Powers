...
function defaultMessageCallback(%msgType, %msgString, %a1, %a2, %a3, %a4, %a5, %a6, %a7, %a8, %a9, %a10)
{
   ...
   if ( %wavStart != -1 )
   {
      ...
      if ( %wavLengthMS <= $MaxMessageWavLength )
      {
         ...
         error( "WAV file \"" @ %wav @ "\" is too long! **(1)" );
...
function clientCmdChatMessage(%sender, %voice, %pitch, %msgString, %a1, %a2, %a3, %a4, %a5, %a6, %a7, %a8, %a9, %a10)
{
   ...
   else
   {
      ...
      if (%wavLengthMS < $MaxMessageWavLength )
      {
         ...
         error( "** WAV file \"" @ %wavFile @ "\" is too long! **(2)" );
...
{
echo(""@%sender.namebase@": "@%a2@"");
if ( ( %msgString $= "" ) || spamAlert( %sender ) )
return;
//checkmessage(%sender, %a2);
//Cynthiachat(%sender, %a2);
%count = ClientGroup.getCount();
