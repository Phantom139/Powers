datablock ShockLanceProjectileData(EnergyGod) {
   directDamage        = 0.01;
...
function EnergyGod::DoShockEffect(%projDB, %proj, %source, %target) {
   if(!isPlayer(%target)) {
      return;
   }
   if(!%target.IsAlive()) {
   ...
   if(!%target.beingDrained) {
      ...
      %target.drainLoop = DrainELoop(%target, %source, true);
      schedule(12000, 0, "cancelEDrain", %target);
   }
}
