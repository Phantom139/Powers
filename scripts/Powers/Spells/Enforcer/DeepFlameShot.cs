//FIRE BOLT
datablock ParticleData(DeepFlameBaseParticle) {
...
datablock ParticleEmitterData(DeepFlameBaseEmitter) {
   ejectionPeriodMS = 10;
   periodVarianceMS = 0;
   ...
   particles = "DeepFlameBaseParticle";
};
...
datablock LinearFlareProjectileData(DeepFlame) {
   ...
   baseEmitter        = DeepFlameBaseEmitter;
...
function DeepFlame::onExplode(%data, %proj, %pos, %mod) {
   ...
   while ((%ply = ContainerSearchNext()) != 0) {
      ...
      if(CanAOEHit(%pos, 10, %ply)) {
         if(!PowersRandChance(20)) {
            return;
         }
         else {
            stunObject(%ply, %proj.sourceObject, 5);
         }
      }
   }
}
