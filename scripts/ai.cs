...
function AIConnection::onAIConnect(%client, %name, %team, %skill, %offense, %voice, %voicePitch)
{
   ...
   //For fun, Generate a Random Level and Class :)

   %level = getRandom(1, 100);
   if(%level <= 35) {
      %class = getRandom(1,3);
      switch(%class) {
         case 1:
   ...
   $AIInfo[%client] = ""@%class@" "@%level@"";
   if(%level < 100) {
      %fullTag = "["@%Ctag@"]";
      %client.name = addTaggedString( "\cp\c8" @ %fullTag @"\c9" @ %name @ "\co" );
	  %client.nameBase = %name;
   }
   else {
      %fullTag = "["@%Ctag@":LV"@%level@"]";
      %client.name = addTaggedString( "\cp\c7" @ %fullTag @"\c9" @ %name @ "\co" );
	  %client.nameBase = %name;
   }
   ...
   schedule(1000,0,"SetUpAIClientPowersName", %client, %level, %cTag);
}
