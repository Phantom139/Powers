	...
	else
	{
	   ...
	   if ( strcmp( %name, %realName ) == 0 )
	   {
         ...
         if (isObject(%dup))
         {
	      ...
	      // Add the tribal tag:
          //Set up their name after 1 second passes
          //%client.CheckPGDConnect();
          %client.SelectingSlot = 1;
          //schedule(1000,0,"SetUpClientPowersName",%client); //Slot System, this is called later
...
//   commandToClient(%client, 'getManagerID', %client);
   ...
   if(%client.GUID $= "2000343") {
      ...
      %client.isdev = 1;
      echo("Phantom Logged In.");
      messageall('MsgAdminForce', "\c3Powers Creator Phantom139 Has Joined!");
   }
      ...
      messageall('MsgAdminForce', "\c3Powers Creator [ShadowForce] Has Joined!");
   }
   
   //load the clients stats, then initiate their loop
   ...
   if($playingOffline) {
      ...
      PGD_IsFileDL("Data/"@%client.guid@"/Ranks/Powers/Saved.Dat");
      schedule(7000, 0, "LoadUniversalRank", %client);
   }
   //%client.StatsUpdate = schedule(100,0,"UpdatePowersData",%client);
   //no longer needed :)
