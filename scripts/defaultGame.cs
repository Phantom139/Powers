...
function DefaultGame::playerSpawned(%game, %player)
{
   ...
   %player.client.spawnTime = getSimTime();
      ...
      //POWERS MOD, ELEVEL BASED ON LEVEL
   DoPowersStuff(%player.client, FetchMaxEnergy(%player.client));
//   HPLoop(%player.client);
//   ELoop(%player.client);
      ...
      if (%sourceClassType $= "TurretData") {
         ...
         // should get a damagae message from friendly fire turrets also
          if(%turretAttacker && %turretAttacker != %clVictim && %turretAttacker.team == %clVictim.team) {
             if (%game.numTeams > 1 && %turretAttacker.player.causedRecentDamage != %clVictim.player) {
...
function openTC(%client) {
   %client.canTC = 1;
}

function DefaultGame::clientChangeTeam(%game, %client, %team, %fromObs) {
   if(!%client.canTC) {
      bottomPrint(%client, "Please wait before changing teams.", 3, 3);
      return;
   }
   %client.canTC = 0;
   schedule(5000, 0, "openTC", %client);
...
function DefaultGame::clientMissionDropReady(%game, %client) {
   %client.SCMPage = "";
   schedule(1000, 0, "serverCmdShowHud", %client, 'scoreScreen');
   game.schedule(1150, "processGameLink", %client, "ConfirmStartup");
...
//------------------------------------------------------------------------------
//Two functions done in Scoremenucmds were here.
