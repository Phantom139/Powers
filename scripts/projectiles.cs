...
function RadiusExplosion(%explosionSource, %position, %radius, %damage, %impulse, %sourceObject, %damageType)
{
   ...
   for (%i = 0; %i < %numTargets; %i++)
   {
      ...
      else if( (%explosionSource.getDataBlock().getName() $= "ConcussionGrenadeThrown" || %explosionSource.getDataBlock().getName() $= "ConcussionBlowProjectile") && %data.getClassName() $= "PlayerData" )
	  {
